...
import { sampleGeometrySurface, quaternionFromUpToNormal, } from "../utils/sampleGeometry";
import vertexShader from "../shaders/grass.vert.glsl?raw";
import fragmentShader from "../shaders/grass.frag.glsl?raw";
export const Grass = ({ sourceGeometry = null }) => {
    const { bladeCount, patchSize, bladeHeight, bladeWidth, windStrength, noiseFreq, noiseAmp, colorBottom, colorTop, curvature, timeScale, followNormals, waveAmp, waveLength, waveSpeed, waveDirectionDeg, waveBlend, wireframe, } = useControls();
    ...
    if (!uniformsRef.current) {
        uniformsRef.current = {
            ...
            uWaveDir: {
                value: new THREE.Vector2(Math.cos((waveDirectionDeg * Math.PI) / 180), Math.sin((waveDirectionDeg * Math.PI) / 180)),
            },
        ...
        window.__grassUniforms = uniformsRef.current;
        window.__grassInteractEnabled = true;
    }
    const geometry = useMemo(() => {
        ...
        for (let i = 0; i < bladeCount; i++) {
            ...
            if (sampled) {
                ...
                tmpNormal
                    .set(sampled.normals[i * 3], sampled.normals[i * 3 + 1], sampled.normals[i * 3 + 2])
                    .normalize();
            }
            else {
        ...
        instGeom.setAttribute("aQuat", new THREE.InstancedBufferAttribute(quats, 4));
    ...
    return (_jsxs("group", { children: [_jsx("mesh", { ref: meshRef, geometry: geometry, frustumCulled: false, children: _jsx("rawShaderMaterial", { ref: materialRef, vertexShader: vertexShader, fragmentShader: fragmentShader, uniforms: uniformsRef.current, side: THREE.DoubleSide, wireframe: false }) }), sourceGeometry ? (_jsx("mesh", { geometry: sourceGeometry, receiveShadow: true, castShadow: true, onPointerMove: (e) => {
                    e.stopPropagation();
                    if (!uniformsRef.current)
                        return;
                    if (window.__grassInteractEnabled === false)
                        return;
                ...
                }, children: _jsx("meshStandardMaterial", { color: colorBottom, roughness: 0.9, metalness: 0.0 }) })) : (_jsxs("mesh", { rotation: [-Math.PI / 2, 0, 0], receiveShadow: true, onPointerMove: (e) => {
                    e.stopPropagation();
                    if (!uniformsRef.current)
                        return;
                    if (window.__grassInteractEnabled === false)
                    ...
                    if (!uniformsRef.current)
                        return;
                    uniformsRef.current.uInteractorEnabled.value = 0;
                }, children: [_jsx("planeGeometry", { args: [patchSize, patchSize, 1, 1] }), _jsx("meshStandardMaterial", { color: colorBottom, roughness: 0.95 })] }))] }));
};
