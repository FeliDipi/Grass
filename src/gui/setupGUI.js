...
export function setupGUI() {
    ...
    const update = (key) => (value) => useControls.getState().set({ [key]: value });
    gui
    ...
    inter
        ...
        .name("Enabled")
        .onChange((v) => {
        window.__grassInteractEnabled = v;
        const uniforms = window.__grassUniforms;
        if (uniforms)
            uniforms.uInteractorEnabled.value = v ? 1 : 0;
    });
    inter
        ...
        const uniforms = window.__grassUniforms;
        if (uniforms)
            uniforms.uInteractorRadius.value = v;
    });
    inter
        ...
        const uniforms = window.__grassUniforms;
        if (uniforms)
            uniforms.uInteractorStrength.value = v;
    });
    return gui;
}
